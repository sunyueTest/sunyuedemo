/******************************************************************************
	jquery.transformable.js v0.3.5 copyright 2017 Aaron Flin (aaron at flin dot org)
...
//console.log("originusesborder="+originusesborder);
	...
	function enablescale(tf) {
		...
		//moved to upper left, - forget double click thing
			tf.append('<div class="transformable-handle-scale transformable-handle zt-icon" title="缩放"></div>');
...
*/
	...
	function enableskew(tf) {
		...
		tf.append('<div title="倾斜 X轴" class="transformable-handle-skew-h transformable-handle-skew transformable-handle zt-icon"></div><div class="transformable-handle-skew-hl transformable-handle zt-icon"></div>');
		tf.append('<div title="倾斜 Y轴"class="transformable-handle-skew-v transformable-handle-skew transformable-handle zt-icon"></div><div class="transformable-handle-skew-vl transformable-handle zt-icon"></div>');
	...
	function enablerotate(tf) {
		tf.append('<div title="旋转" class="transformable-handle-rotate transformable-handle zt-icon"></div>');
	...
	jQuery.ui.resizable.prototype._mouseDrag=function(event) {
		...
		if (c&&c.transformed) points=$.singleLevelTP(points,{x:event.pageX,y:event.pageY},imm(c.totalmatrix));
...
//console.log(event);
	...
	$.fn.tDrag= function(x1,y1,ui) {
	   ...
	   self.each(function(i){
		...
		if(t.hasClass('ui-draggable-dragging') && !t.data("ui-draggable").containment) range=false;
		var delta;
		if (range) {
...
//console.log(off);
			...
			if(options===true) {
				...
				var bw={top:parseInt(s.borderTopWidth), left:parseInt(s.borderLeftWidth)};
			...
			if (tm) {
				...
				if (options===true) innercorners=$.singleLevelTP(innercorners,center,tm);
			}
			else tm=[1,0,0,1,0,0];
			var sm=tm;
			...
			while(par.length&&!par.is('body')&&(!selector||!par.is(selector))) {
				...
				if (m) {
					...
					if(options===true) innercorners=$.singleLevelTP(innercorners,cn,m);
					tm=mm(tm,m);
				}
				par=par.parent();
			}
...
*/
	...
	jQuery.fn.transformable.offsetNoGetBounds = function( options ) {
		...
		if(parseFloat($.fn.jquery.match(/^\d+\.\d+/))<1.7) jQuery.offset.initialize();
