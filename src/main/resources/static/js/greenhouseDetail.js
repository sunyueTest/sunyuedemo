...
$(function () {
    ...
    $(".landLineItemBox").height($("#landLine").height());
            ...
            if (data.state == "success" && data.datas.length > 0) {
                ...
                for (var i = 0; i < data.datas.length; i++) {
                    if (data.datas[i].deviceType != 2) {
                        ...
                        initLandLine(data.datas[i].deviceNumber);
...
function addDeviceMsg(res) {
    ...
    if (itemValue && itemValue.length > 0) {
        ...
        $environmental.empty();
        var $landLineItemBox = $(".landLineItemBox");
        $landLineItemBox.empty();
        //加载环境实时监测数据
        ...
        //加载土地墒情右侧格子数据
        for (var x = 0; x < itemValueArr.length; x++) {
            for (var c = 0; c < landLegData.length; c++) {
                if ($.trim(itemNameArr[x]) == $.trim(landLegData[c])) {
                    var $landMsgItem = $("<div class='landMsgItem'></div>");
                    $landMsgItem.addClass(xArr[x]);
                    if (itemValueArr[x].length > 0) {
                        $landMsgItem.text($.trim(itemNameArr[x]) + ":" + $.trim(itemValueArr[x]));
                    } else {
                        $landMsgItem.text($.trim(itemNameArr[x]) + ":" + "暂无数据");
                    }
                    $landMsgItem.appendTo($landLineItemBox);
                    break;
                }
            }
                ...
                if (keyArr.length > 0) {
                    ...
                    if (server.length > 0) {
                        ...
                        option = {
                            ...
                            legend: {
                                color: ["#afeef2", "#afcbf2", "#e0aff2", "#f2d2af", "#e0aff2", "#009944", "#ff0000", "#13e1f3", "#afeef2", "#afcbf2", "#e0aff2", "#f2d2af", "#e0aff2", "#009944", "#ff0000", "#13e1f3"],
                                data: landLegData,
                                top: '10%',
                                left: 'center',
                                itemWidth: 1,//图例的宽度
