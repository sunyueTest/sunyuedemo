    ...
    layui.use('table', function () {
        ...
        table.on('row(sensor)', function (obj) {
            ...
            if (rowData.sensorName.trim() == '经度' || rowData.sensorName.trim() == '纬度') {
                $.post("../newTrajectoryReplay/checkDeviceHasLonLat", {"deviceNumber": rowData.sensorNcode}, function (res) {
                    if (res.state == 'success') {
                        parent.parent.newJumpTrajectoryReplay(rowData.sensorNcode);
                    } else {
                        layui.use('layer', function () {
                            layer.msg("该设备没有经纬度信息");0
                        });
                    }
                });
            }
