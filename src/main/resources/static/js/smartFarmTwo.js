...
function initAttendanceLine(dataId) {
    var myChart = echarts.init(document.getElementById('attendanceLine'));
...
findFarmDetails = function (num) {
    // 获取当前农场下所有设备实时信息
    currencyFarmDeviceList(farmList[num].id);
    //所属农场的农作物生长周期
    findFarmCropsLists(farmList[num].id);
    //获取农场对应摄像头信息
    findLiveAddress(farmList[num].cameraNumber);
    //添加地图上农场的基本信息
    ...
    if (currencyFarmDeviceListValue.state == 'success'&&currencyFarmDeviceListValue.datas.length>0) {
        ...
        for (var k = 0; k < typeList.length; k++) {
            str = '<div class="itemBox">' +
                ...
                '<span class="itemName">' + typeList[k] + '</span>' +
                '<span class="itemValue">' + dataList[k] + '</span>' +
                '</div>' +
                '</div>';
 ...
 * 加载对应农场的摄像头
 */
function getCamareUrl(liveAddressValue) {
    ...
    if (liveAddressValue.state == 'success') {
        var data = liveAddressValue.data;
        ...
        $("#playerNoData").find(".nomsg").text(liveAddressValue.msg).css("display", "block");
    }
}
...
findDeviceLine = function (deviceNumber) {
    var myChart = echarts.init(document.getElementById('landLine'));
    myChart.clear();
    var server = [];
    var legData = [];
...
function initLandBar(landBar) {
    ...
    if (landBar.state == 'success' && landBar.data.length > 0) {
        ...
        var nums = 100;
        if (k > 0) {
            nums = ((data2[k] - data2[k - 1]) / data2[k - 1]).toFixed(2) * 100;
        ...
        $(".topCenterOneRight").text("种植度" + num + nums + last);
