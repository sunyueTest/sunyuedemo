...
TWEEN.Tween = function (e) {
    var t = e, n = {}, r = {}, i = {}, s = 1e3, o = 0, u = null, a = TWEEN.Easing.Linear.EaseNone, f = null, l = null,
        c = null;
        ...
        for (var a = 0, f = this._selector.getCount(); a < f; ++a) {
            ...
            for (var l = 0, c = e.length; l < c; ++l) {
                ...
                for (var h = 0, p = s.length; h < p; ++h) {
                    ...
                    if (h == 0) {
                        if (n.delay == null) n.delay = this._part_delay + r + this._startDelay; else n.delay += this._part_delay + r + this._startDelay
                    }
...
Aroma.Effect.prototype.getFromData = function () {
    if (this.fromData != null && this.isStatic) return this.fromData; else if (this.isStatic) {
...
(function () {
    ...
    if (window.addEventListener) window.addEventListener("resize", e); else if (window.attachEvent) window.attachEvent("onresize", e);
...
Cute.FallBack.prototype = {
    force: null, __result: null, getType: function () {
        ...
        switch (n) {
            ...
            case"linux":
            case"ubuntu":
                if (r == "chrome" || r == "safari" || r == "chromium" || e.engine.name == "AppleWebKit") t = Cute.FallBack.CSS3D; else if (r == "ie" && parseInt(e.browser.major) >= 9 || r == "firefox" || r == "opera") t = Cute.FallBack.CANVAS;
                break;
            case"ios":
        ...
        if (window.Modernizr) {
            if (t == Cute.FallBack.CANVAS && !Modernizr.canvas) t = Cute.FallBack.DOM2D; else if (t == Cute.FallBack.CSS3D && !Modernizr.csstransforms3d) t = Cute.FallBack.DOM2D; else if (i) {
                if (Modernizr.csstransforms3d) t = Cute.FallBack.CSS3D; else if (Modernizr.canvas) t = Cute.FallBack.CANVAS
            }
        }
        this.__result = t;
        return t
...
Cute.ModuleLoader.css3d_files = ["/public/screenSaver/cute.css3d.module.js"];
Cute.ModuleLoader.canvas_files = ["/public/screenSaver/cute.canvas.module.js"];
Cute.ModuleLoader.dom2d_files = ["/public/screenSaver/cute.2d.module.js"];
...
Averta.ScrollContainer.prototype = {
    constrcutor: Averta.ScrollContainer, setup: function () {
        ...
        function s(n) {
            ...
            if (document.detachEvent) document.detachEvent("onmousemove", e.element); else document.removeEventListener("mousemove", e.element, false)
        }
...
Cute.ItemList = function (e) {
    ...
    this.__scrolling = function () {
        if (t.type == "vertical") t.sc.translate(0, n); else t.sc.translate(-n, 0)
    };
...
Cute.SlideManager.prototype = {
    constructor: Cute.SlideManager, startTimer: function () {
        ...
        if (this._timer.getTime() >= this._currentSlide.delay * 1e3) {
            this._timer.stop();
            if (this._nextSlide.ready) this.showSlide(this._nextSlide); else this.waitForNext()
        }
        this._delayProgress = this._timer.getTime() / (this._currentSlide.delay * 10);
        this.dispatchEvent(new Cute.SliderEvent(Cute.SliderEvent.WATING))
    }, prepareTransition: function (e) {
        if (this.rotator.fallBack.getType() == Cute.FallBack.DOM2D) return e.transitions2D[parseInt(Math.random() * e.transitions2D.length)]; else return e.transitions3D[parseInt(Math.random() * e.transitions3D.length)]
        ...
        if (this._currentSlideIndex + 1 == this._slideList.length) return 0; else return this._currentSlideIndex + 1
    }, getPreviousIndex: function () {
        if (this._currentSlideIndex - 1 == -1) return this._slideList.length - 1; else return this._currentSlideIndex - 1
        ...
        this._autoPlay = e;
        if (!this._autoPlay) this._timer.stop(); else this._timer.start();
...
Cute.SlideControl.Point.prototype = {
    constructor: Cute.SlideControl.Point, align: "bottom", changeSlide: function () {
        ...
        if (this.sc.config.thumb_align == "up") this.thumb.domElement.style.top = 10 - this.thumb.frame.offsetHeight + -this.thumb_pos * .1 + "px"; else this.thumb.domElement.style.top = 24 + -this.thumb_pos * .1 + "px"
    }, showThumb: function () {
...
Cute.SlideInfo.prototype.update = function () {
    if (this.data) {
        if (this.eff == "fade") setOpacity(this.content, this.position); else this.content.style[this.data.align] = this.position + "px"
    }
};
Cute.SlideInfo.prototype.show = function () {
    ...
    if (this.eff == "slide") this.position = -(this.data.align == "bottom" || this.data.align == "top" ? this.content.offsetHeight : this.content.offsetWidth); else this.position = 0;
...
Cute.TouchNavigation = function (e, t) {
    ...
    this.__touchEnd = function (n) {
        ...
        r = false;
        clearTimeout(o);
        if (s - i > e.offsetWidth / 10) t.next(); else if (s - i < -e.offsetWidth / 10) t.previous();
        i = s = 0
    };
    if (n) {
...
Cute.Slider.prototype = {
    constructor: Cute.Slider, setup: function (e, t, n) {
        ...
        for (var i = 0, s = r.length; i < s; ++i) {
            if (r[i].getAttribute("data-type") == "slides") this.slidesElement = r[i]; else if (r[i].getAttribute("data-type") == "controls") this;
        ...
        this.slideManager.height = documentHeight;//this.wrapper.clientWidth / this.aspect;
        ...
        this.element.style.height = documentHeight + "px"//this.slideManager.height
    }, __onresize: function () {
        this.__setSize();
        this.slideManager.resize()
